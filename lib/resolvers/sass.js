 ...
 * @typedef {import('scss-parser').Node} ScssParserNode
 * @typedef {import('./node.js').NodeResolverOptions} NodeResolverOptions
 */
...
class SassResolver extends NodeResolver {
	...
	isStaticString(value) {
		...
		return !hasInterpolationOrVariable(parsed);
	}
}

/**
 * @param   {ScssParserNode} parsed
 * @returns {boolean}
 */
function hasInterpolationOrVariable(parsed) {
	if (Array.isArray(parsed.value)) {
		return parsed.value.some((item) => {
			if (
				['interpolation', 'variable'].includes(item.type) ||
				(['string_double', 'string_single'].includes(item.type) &&
					typeof item.value === 'string' &&
					item.value.includes('#{'))
			) {
				return true;
			}
			return hasInterpolationOrVariable(item);
		});
	}
	return false;
}
